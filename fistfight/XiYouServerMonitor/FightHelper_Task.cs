  ...
  public class FightHelper_Task   
    {
      ...
      /// <returns></returns>
      public V_xy_sp_userView EnemyStartFight(V_xy_sp_userView userView,ref string batmsgAll)
      {

          V_xy_sp_userspirit FighterTo=userView.Spirit;
          List<V_xy_sp_spirit> FighterFromList=userView.Task.SpiritsList;

          List<V_xy_sp_spirit> FighterToResult = new List<V_xy_sp_spirit>();
           
               int maxInjuryCount = 0;
               foreach (V_xy_sp_spirit spirit in FighterFromList)
               {
                   if (spirit.SpiritLife <= 0)
                   {
                       continue;//生命值低于0跳过
                   }
                   V_xy_sp_skill skill = spirit.spiritSkillList[RandomSkill(spirit.spiritSkillList.Count)];
                   if (skill.isGroupinjury == 1 && maxInjuryCount == FighterFromList.Count)
                   {
                   ...
                   FighterToResult.Add(spirit);
                   //所有怪物已阵亡
                   if(spirit==FighterFromList.LastOrDefault())
                   {
                       if(spirit.SpiritLife<=0)
                       {
     ...
     /// <returns></returns>
      ...
      /// <param name="rate"></param>
      /// <returns></returns>
      public static int RandomSkill(int skillLenth)
      {
          Random r = new Random();
          int result = r.Next(skillLenth);
          return result;
      }
